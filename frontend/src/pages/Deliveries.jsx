...
export default function Deliveries() {
  ...
  const { data: deliveries, isLoading: loadingDeliveries } = useQuery({
    queryKey: ["deliveries"],
    queryFn: operationService.getDeliveries,
  });
  ...
  const { data: customers } = useQuery({
    queryKey: ["customers"],
    queryFn: contactService.getCustomers,
  });
  ...
  const { data: customerLocations } = useQuery({
    ...
    queryFn: () => locationService.getAll({ type: "CUSTOMER" }),
  });
  ...
  const validateMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.message || "Failed to validate delivery"
      );
    },
  });
  ...
  const handleValidate = (id) => {
    if (
      window.confirm(
        "Are you sure you want to validate this delivery? This will move stock."
      )
    ) {
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Deliveries</h1>
        {!isAdding && (
          <button
      ...
      {isAdding && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 mb-6 border border-gray-200 dark:border-slate-700">
          <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Create New Delivery</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                >
                  ...
                  {customerLocations?.data?.map((loc) => (
                    <option key={loc._id} value={loc._id}>
                      {loc.name}
                    </option>
                  ))}
              ...
              {formData.products.map((item, index) => (
                <div key={index} className="flex gap-2 mb-2">
                  ...
                  <input
                    ...
                    className="w-32 border border-gray-300 dark:border-slate-600 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-white"
                    min="1"
                    step="1"
                    required
                  />
